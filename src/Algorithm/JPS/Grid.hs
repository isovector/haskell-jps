...
instance Show Grid where
  show (Grid (GridDims xmax ymax) _) =
    "(" ++ show xmax ++ " x " ++ show ymax ++ ") Grid"
...
instance Show SearchNode where
  show (SearchNode pr c _ dep) =
    "(SearchNode: { " ++ (show pr) ++ "," ++ (show c) ++ "," ++ (show dep) ++ " })"
...
newPathfinding grid start finish =
  Pathfinding grid Map.empty PSQ.empty start finish
